...
const ServerCard: React.FC<ServerCardProps> = ({ server, onStart, onStop, onDelete }) => {
    ...
    return (
        <div className="card server-card" onClick={handleCardClick} style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: '15px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ margin: 0, fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{ width: '10px', height: '10px', borderRadius: '50%', backgroundColor: getStatusColor(server.status) }}></div>
                    {server.name}
                </h3>
                <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', border: '1px solid var(--border-color)', padding: '2px 8px', borderRadius: '4px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: 'auto' }}>
                {server.status === 'STOPPED' ? (
                    <button
                        ...
                        style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', backgroundColor: '#166534' }}
                    >
                        <Play size={16} /> Start
                    </button>
                ) : (
                    <button
                        ...
                        style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', backgroundColor: server.status === 'STARTING' ? '#854d0e' : '#991b1b', cursor: server.status === 'STARTING' ? 'not-allowed' : 'pointer', opacity: server.status === 'STARTING' ? 0.7 : 1 }}
                    >
                        <Square size={16} /> Stop
                    </button>
                )}
